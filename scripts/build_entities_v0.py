...
def clean_canonical_name(company_name: str) -> str:
    """Conservatively clean real source company names only."""
...
def build_entities(rows: list[dict[str, str]], fieldnames: list[str]) -> tuple[list[dict[str, str]], list[str]]:
    ...
    for index, row in enumerate(sorted_rows, start=1):
        ...
        source_company_name = normalize_text(row.get("company_name")) if has_company_name else ""
        used_ticker_fallback = not source_company_name
        if used_ticker_fallback:
            company_name = ticker
